    ...
    <>
      <section className="alPageContainer">
        <Card
          ...
          albumYear={getOnlyYear(albumData.release_date)}
        />
      </section>
      <section className="tracksCard">
        {albumData.tracks.items.map((music, index) => (
          <Tracks
            ...
            trackNumber={index + 1}
            trackName={
              music.name.slice(0, 26) + (music.name.length > 26 ? "..." : "")
            }
            albumName={
              music.artists.slice(0, 26) + (music.name.length > 26 ? "..." : "")
            }
            musicLength={(music.duration_ms / 60000).toFixed(2)}
          />
        ))}
      </section>
    </>
